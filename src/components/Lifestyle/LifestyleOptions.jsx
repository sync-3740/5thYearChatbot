...
const LifestyleOptions = (props) => {
  const options = [
    ...
    { text: "Diet", handler: () => {props.actionProvider.handleRequest("Diet"); props.actionProvider.handleNewVideoMessage("A healthy and balanced diet can help you lower and control your blood pressure, help with weight management, and decrease your risk of developing heart conditions. Try to make sustainable healthy changes to your diet. Crash diets aimed at losing weight rarely lead to permanent results and don't help you build healthy habits.", "https://www.youtube.com/embed/XgioHTbJgCc")}, id: 2 },
    { text: "Salt", handler: () => {props.actionProvider.handleRequest("Salt"); props.actionProvider.handleNewVideoMessage("Salt, which is sometimes also called sodium, is necessary in your diet in small amounts. Too much of it, however, can affect your body and increase your blood pressure. Adults should eat no more than 6 grams of salt a day, which about one level teaspoon.", "https://www.youtube.com/embed/vEm6bfDRV3w")}, id: 3 },
    { text: "Fats", handler: () => {props.actionProvider.handleRequest("Fats"); props.actionProvider.handleNewVideoMessage("Alongside being present in commercially processed foods, fats are naturally found in lots of the plant- and animal-based foods that we eat.  Fats are good for the body in small amounts, as they are a great source of energy and our bodies need them to function properly. However, too many saturated fats can increase the risk of having heart attacks and strokes.", "https://www.youtube.com/embed/5oy0Tl8GA8I")}, id: 4 },
    ...
    { text: "Smoking", handler: () => {props.actionProvider.handleRequest("Smoking"); props.actionProvider.handleNewVideoMessage("It's widely known that smoking is very damaging to our health. Smoking causes about 90% of lung cancers, and it also causes cancer in many other parts of the body, including the mouth, throat, bladder, liver, stomach, and pancreas. But how does smoking damage your heart and circulation?", "https://www.youtube.com/embed/cuG9aVnZf-A"); props.actionProvider.handleNewWidgetMessage("More information about smoking can be found by clicking on the following options:", "SmokingOptions")}, id: 6 },
